---
name: kyberis
description: Guides Cursor Agent mode to query Kyberis through MCP or API, resolve entities, look up evidence, find relationships and ATT&CK detection knowledge, prioritize threats, run structured assessments, and produce security recommendations. Use when users ask for Kyberis, threat lookup, incident investigations, evidence lookup, entity resolution, relationship pivots and ATT&CK detection guidance, IOC/CVE/actor assessment, environment prioritization, or remediation guidance.
---
...
2. Run environment assessment with `environment_context.products` and any supplied exposure/control signals. Preserve the returned applicability and verification steps.
